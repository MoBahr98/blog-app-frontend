...
const allowedTags = BLOG_NAVBAR_DATA.map((item) => item.label.toLowerCase());

const TagInput = ({ tags, setTags }) => {
  const [input, setInput] = useState("");

  const handleKeyDown = (e) => {
    if ((e.key === "Enter" || e.key === ",") && input.trim().toLowerCase() && allowedTags.slice(1).includes(input.trim().toLowerCase())) {
      e.preventDefault();
